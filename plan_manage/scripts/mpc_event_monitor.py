...
class MpcEventMonitor:
    ...
    def metrics_text(self):
        ...
        best_clearance = self.fmt(data[9]) if len(data) > 9 else "NA"
        best_ttc = self.fmt(data[10]) if len(data) > 10 else "NA"
        return ("valid={} best_clearance={} best_ttc={} "
                "global_clearance={} global_ttc={} plan_valid={}").format(
            valid, best_clearance, best_ttc,
            clearance, ttc, plan_valid)
