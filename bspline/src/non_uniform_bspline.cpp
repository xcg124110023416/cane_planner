...
void NonUniformBspline::parameterizeToBspline(const double& ts, const vector<Eigen::Vector3d>& point_set,
  ...
  // // 速度约束：起点和终点的速度（一阶导数）
  // A.block(K, 0, 1, 3)         = (1 / 2.0 / ts) * vrow.transpose();  // 起点速度
  // A.block(K + 1, K - 1, 1, 3) = (1 / 2.0 / ts) * vrow.transpose();  // 终点速度

  // // 【删除加速度约束】
  // // 原代码的加速度约束（已删除）：
  // // A.block(K + 2, 0, 1, 3)     = (1 / ts / ts) * arow.transpose();  // 起点加速度
  // // A.block(K + 3, K - 1, 1, 3) = (1 / ts / ts) * arow.transpose();  // 终点加速度
  ...
  // // 只使用前两个导数（速度约束）
  // bx(K)     = start_end_derivative[0](0);  // 起点速度 x
  // by(K)     = start_end_derivative[0](1);  // 起点速度 y
  // bz(K)     = start_end_derivative[0](2);  // 起点速度 z（通常为0）
  
  // bx(K + 1) = start_end_derivative[1](0);  // 终点速度 x
  // by(K + 1) = start_end_derivative[1](1);  // 终点速度 y
  // bz(K + 1) = start_end_derivative[1](2);  // 终点速度 z（通常为0）
