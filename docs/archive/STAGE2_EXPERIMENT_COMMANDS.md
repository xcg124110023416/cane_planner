...
含义：启用 Stage 2 scene/debug topic，但不允许 interaction YIELD。用于确认 interaction 观测层是否污染原始行为。
...
```bash
cd /home/xcg/ws && source /opt/ros/noetic/setup.bash && source devel/setup.bash && roslaunch plan_manage sim_kin_replan.launch planner:=3 use_pedestrians:=true pedestrian_scenario:=crossing mpc_w_risk:=2.0 mpc_risk_sigma_y:=0.38 mpc_enable_cpa:=false mpc_enable_stop_advice:=false mpc_enable_stop_enforce:=false mpc_enable_interaction:=true mpc_interaction_enable_yield:=false
```
...
含义：当前 Stage 2 主线。启用时空冲突让行监督层，用机器人未来路径占用和行人预测占用的时间重叠决定是否 YIELD；冲突解除后通常回到 CONTINUE。PASS_BEHIND 和 interaction social cost 已从主流程移除。
...
- `E1`: 期望出现 `CONTINUE -> YIELD -> CONTINUE`，`stop_reason` 为 `INTERACTION_YIELD_CONFLICT`，当前主线不包含 PASS_BEHIND social cost。
